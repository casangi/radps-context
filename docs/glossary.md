...
- **Node task**: A unit of work that can be assigned to a node. It invokes processing functions and consumes and produces data chunks or intermediate artifacts.
...
- **Processing scope**: An identifiable portion of a run's domain state to which state, an update, a processing output, or a decision applies, expressed using stable identifiers such as a run, dataset, data chunk, field, spectral window, antenna, or time range.
...
- **Update identity**: A stable identifier used to recognize retries of the same logical context update. Reusing it for an equivalent outcome returns the previously accepted update; reusing it for a different outcome is an error.
